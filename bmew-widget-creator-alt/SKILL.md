...
**This is the `-alt` variant.** It's identical to `bmew-widget-creator`
except for how the capability index's per-lookup freshness check works — see
[`references/capability-index/README.md`](references/capability-index/README.md)
for what changed and the tradeoff it carries. Everything else in this skill
(domain evidence, wizard doctrine, navigation routes, scripts other than the
freshness check itself) is the same as the main skill; changes made to one
