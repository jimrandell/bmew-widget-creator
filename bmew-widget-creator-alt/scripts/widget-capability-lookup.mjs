...
import { generatorProtocolVersion, statAuthorityInputs, outputPath } from './build-widget-capability-index.mjs';
...
export async function validateIndexFreshness(index, read = readFile, stat = fsStat) {
    if (index.generatorProtocolVersion !== generatorProtocolVersion) return { ok: false, reason: 'generator-protocol-mismatch' };
    try {
        const current = await statAuthorityInputs(stat);
...
export async function createCapabilityLookupSession({ indexPath = outputPath, readFile: read = readFile, stat = fsStat } = {}) {
    const cache = new Map();
    // Load the index and check freshness at most once per session, not once per lookup() call.
    let loaded = null;
    const load = () => loaded ??= (async () => {
        const index = JSON.parse(await read(indexPath, 'utf8'));
        const freshness = await validateIndexFreshness(index, read, stat);
