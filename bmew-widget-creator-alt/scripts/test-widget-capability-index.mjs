...
// -alt: authorityInputs carry size/mtimeMs (a stat), not a sha256 content hash.
assert.equal(index.generatorProtocolVersion, '1-alt-stat-freshness');
assert(index.authorityInputs.every(input => typeof input.size === 'number' && typeof input.mtimeMs === 'number' && !('sha256' in input)), 'authorityInputs must carry size/mtimeMs, not sha256');
