...
// Capability index: protocol version and authority-input freshness.
// -alt: freshness is size/mtimeMs from a stat(), not a content hash — see
// build-widget-capability-index.mjs's statAuthorityInputs for the tradeoff.
const capabilityIndex = JSON.parse(read(join(skillRoot, 'references/capability-index/capability-index.json')));
assert.equal(capabilityIndex.generatorProtocolVersion, '1-alt-stat-freshness', 'Unexpected capability-index generator protocol');
for (const input of capabilityIndex.authorityInputs) {
    const actualStats = statSync(join(skillRoot, input.inputPath));
    assert.equal(actualStats.size, input.size, `Stale authority input size: ${input.inputPath} — rerun build-widget-capability-index.mjs`);
    assert.equal(actualStats.mtimeMs, input.mtimeMs, `Stale authority input mtime: ${input.inputPath} — rerun build-widget-capability-index.mjs`);
}
