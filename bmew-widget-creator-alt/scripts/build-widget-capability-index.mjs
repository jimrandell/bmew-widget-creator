...
// -alt: distinct from the main skill's '1' so an index built by one variant's
// generator is never silently accepted by the other's freshness check — the
// authorityInputs shape below (size/mtimeMs) is not the main skill's (sha256).
export const generatorProtocolVersion = '1-alt-stat-freshness';
...
// -alt variant: freshness is checked by file size and modified-time instead of a content hash —
// a stat() call instead of reading and SHA-256-hashing the full file on every lookup. Cheaper, but
// weaker: it can't detect an edit that happens to preserve both size and mtime, and it's fragile
// across a zip-based redeploy, where every extracted file typically gets one new mtime at
// extraction time regardless of its original content — which can make a freshly-deployed,
// unmodified skill report itself as stale. See capability-index/README.md for the full tradeoff.
export async function statAuthorityInputs(stat = fsStat) {
    return Promise.all(inputs.map(async inputPath => {
        const stats = await stat(join(root, inputPath));
        return { inputPath, size: stats.size, mtimeMs: stats.mtimeMs };
    }));
}
...
export async function generateCapabilityIndex(read = readFile, stat = fsStat) {
    const [corpus, hosts, coverageText, identityText, authorityInputs] = await Promise.all([
        read(join(root, inputs[0]), 'utf8'), read(join(root, inputs[1]), 'utf8'), read(join(root, inputs[2]), 'utf8'), read(join(root, sourceIdentityInputPath), 'utf8'), statAuthorityInputs(stat),
    ]);
    return {
...
export function validateGeneratedIndex(index) {
    ...
    for (const input of index.authorityInputs ?? []) if (!input.inputPath || typeof input.size !== 'number' || typeof input.mtimeMs !== 'number') throw new Error('Invalid authority input.');
